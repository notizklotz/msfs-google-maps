...
const routeSourceId: string = 'route';
...
export default class Maptiler extends BaseMap {
    ...
    private addAirportsLayer(map: Map) {
        ...
        map.on('click', 'airports', (e: MapMouseEvent & { features?: Feature[] } & Object) => {
            ...
            if (feature) {
                ...
                parent.updateSelectedAirportData();
                ...
                parent.airportPopup = new Popup()
                    .setLngLat({ lng: coordinates[0], lat: coordinates[1] })
                    .setHTML(
                        `<div id="OSM-popup-data"><h2>${airport.name}</h2><b>${airport.ident}<br>${airport.type.replace(
                            '_',
                            ' ',
                        )}<div id="dynamic-airport-data"></div></b></div>`,
                    )
                    .addTo(parent.map);
            }
        });
